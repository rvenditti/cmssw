...
#create collection of tight muon candidates 
tightMuonsForZMuSkim = cms.EDFilter("PATMuonSelector",
                                    src = cms.InputTag("MyPatMuons"),       
                                    cut = cms.string('(pt > 28) &&  (abs(eta)<2.4) && (isPFMuon>0) && (isGlobalMuon = 1) && (globalTrack().normalizedChi2() < 10) && (globalTrack().hitPattern().numberOfValidMuonHits())&& (numberOfMatchedStations() > 1)&& (innerTrack().hitPattern().numberOfValidPixelHits() > 0)&& (innerTrack().hitPattern().trackerLayersWithMeasurement() > 5) && (abs(dB)<0.2)  && ((isolationR03().sumPt/pt)<0.4)'),
                                    filter = cms.bool(True)                                
                                    )


                                    
#create diMuon candidates
dimuonsZMuSkim = cms.EDProducer("DeltaRMinCandCombiner",
    decay = cms.string('tightMuonsForZMuSkim looseIsoMuonsForZMuSkim'),
    checkCharge = cms.bool(False),
    cut = cms.string('mass > 60 &&  charge=0 && (abs(daughter(0).vz - daughter(1).vz) < 0.5)'),
